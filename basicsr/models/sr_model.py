...
class SRModel(BaseModel):
    ...
    def feed_data(self, data):
        ...
        if 'gt_depth' in data:
            self.gt_depth = data['gt_depth'].to(self.device)
    ...
    def grids(self):
        ...
        self.original_size = self.gt.size()
        assert b == 1
    ...
    def optimize_parameters(self, current_iter):
        ...
        if self.opt['train'].get('depth_weight_matrix',False):
            ...
            base_weight = 2.5  # 在 gt_depth 为0.7时的目标值，保证在3以下
            beta = 0.3          # 控制变化速率的参数
            gamma = 0.7        # 中心点
            ...
            # # 计算 depth_weight_matrix #try1
            # self.depth_weight_matrix[mask] = 1 + (base_weight - 1) * torch.exp(-beta * (self.gt_depth[mask] - gamma)**2)
            self.depth_weight_matrix[mask] = 1.5 + (1 - 2*torch.log10(self.gt_depth[mask])) / 2
